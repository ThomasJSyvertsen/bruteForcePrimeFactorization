...
public class PrimeFactorization implements Iterable<PrimeFactor> {
	...
	public PrimeFactorization(PrimeFactorization pf) {
		...
		while (pfi.hasNext()) {
			Node n = new Node(pfi.next());
			this.add(n.pFactor.prime, n.pFactor.multiplicity);
		}
	}
	...
	public void multiply(long n) throws IllegalArgumentException {
		...
		PrimeFactorizationIterator thisi = this.iterator();
		long m = n;
		int mul = 0;

		for (int i = 2; i * i < m && isPrime(i); i++) {
			// Updates the multiplicity of PrimeFactor pf.
			while (m % i == 0) {
				mul++;
				m = m / i;
			}
			...
			// Places PrimeFactorizationIterators cursor in the right position.
			while (thisi.cursor.pFactor.prime < i && thisi.cursor != this.tail) {
				thisi.next();
			}

			// Multiply PrimeFactorization this at pFactor i with the new PrimeFactorization
			// pf at pFactor i.
			if (thisi.cursor.pFactor.prime == i) {
				thisi.cursor.pFactor.multiplicity += mul;
			} else if (thisi.cursor.pFactor.prime > i || thisi.cursor == this.tail) {
				PrimeFactor p = new PrimeFactor(i, mul);
				thisi.add(p);
			}
		}
	}
